...
public class PostgresTagRepository : ITagRepository
{
    ...
    public async Task SaveArticleTagsAsync(int articleId, List<string> tags, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var tagName in tags)
        {
            ...
            if (existingTag == null)
            {
                existingTag = new TagEntity { Name = tagName, CreatedAt = DateTime.UtcNow };
        ...
        foreach (var tag in tagEntities)
        {
            ...
            {
                _context.ArticleTags.Add(new ArticleTagEntity
                {
                    ArticleId = articleId,
                    TagId = tag.Id,
    ...
    private async Task UpdateTagCoOccurrencesAsync(List<TagEntity> tags, CancellationToken cancellationToken)
    {
        ...
        for (int i = 0; i < tags.Count; i++)
        {
            for (int j = i + 1; j < tags.Count; j++)
            {
                ...
                {
                    _context.TagCoOccurrences.Add(new TagCoOccurrenceEntity
                    {
                        Tag1Id = tag1Id,
                        Tag2Id = tag2Id,
                        OccurrenceCount = 1,
